...
interface IProps {
  open: boolean;
  size: REDUX_STORE.ProductSize | null;
  onClose: () => void;
}
...
const EditSizeModal: React.FC<IProps> = (props) => {
  const { open, onClose, size } = props;
  ...
  const onFormSubmit = useCallback(
    async (values: FormikValues, formikHelpers: FormikHelpers<FormikValues>) => {
      ...
      try {
        ...
        await dispatch(AppActions.updateSizeAction({ id: size.id, name: values.name }));
        onClose();
